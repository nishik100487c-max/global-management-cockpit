...
if "selected_region" not in st.session_state: st.session_state.selected_region = "EMEA" 
...
if "messages_strategy" not in st.session_state:
    st.session_state.messages_strategy = [{
        "role": "assistant",
        "content": "**System Online.** 経営参謀エージェントです。公式ドキュメント（レポート・会議録等）を接続したビジネスオントロジー（構造モデル）との同期を完了しました。\n\n⚠️ **異常検知:** EMEA地域のHardware事業において、利益(EBITDA)が計画比で大幅未達（計画25.0Mに対し実績見込5.0M）となっています。要因の深掘り分析を実行しますか？"
    }]
...
st.markdown("""
    <style>
        ...
        /* サマリーパネル追加スタイル */
        .summary-panel { background: linear-gradient(145deg, #1E293B, #0F172A); border-left: 5px solid #EF4444; padding: 18px 20px; border-radius: 8px; margin-bottom: 15px; border: 1px solid #334155; box-shadow: 0 4px 6px rgba(0,0,0,0.3); }
...
biz_data = {
    ...
    "North America": {"r_p":[250, 130, 80],  "r_a":[245, 135, 70],  "r_py":[230, 120, 70],  "e_p":[35, 25, 8],  "e_a":[34, 25, 6],  "e_py":[30, 22, 8]},
    "EMEA":          {"r_p":[160, 100, 50],  "r_a":[150, 105, 45],  "r_py":[150, 90, 40],   "e_p":[25, 15, 5],  "e_a":[5,  18, 2],  "e_py":[20, 15, 6]},
    "APAC":          {"r_p":[150, 90, 50],   "r_a":[160, 95, 50],   "r_py":[130, 80, 50],   "e_p":[18, 15, 7],  "e_a":[20, 16, 6],  "e_py":[15, 14, 6]},
    "LatAm":         {"r_p":[80, 50, 20],    "r_a":[75, 45, 20],    "r_py":[70, 40, 20],    "e_p":[10, 8, 2],   "e_a":[8,  8, 2],   "e_py":[8,  5, 2]}
}
...
def draw_horizontal_bar(categories, plan, act, py, selected_cat=None):
    ...
    for c, a, p in zip(categories, act, plan):
        ...
        if selected_cat and c != selected_cat:
            colors.append("rgba(100, 116, 139, 0.3)") # 選択外グレーアウト
        else:
...
💡 **推奨アクション**
外部要因による巨大な待機費用を圧縮するため、現在滞留している在庫の一部を需要が逼迫しているAPACへ転送（在庫融通）する方針が有効です。左側の**「🛠️ 3. 収益構造シミュレーション」**タブから、振替によるリカバリー額のシミュレーションを実行してください。
"""
...
with st.expander("💡 このダッシュボードの見方 & 次のステップ（高度分析への誘導）", expanded=False):
    ...
    * **🧠 右側 AI Agent**: 専門家の知見に基づき、事業構造やKPIの因果関係を定義した**「ビジネスオントロジー（構造モデル）」**を基盤に活用。この構造に公式の週報や会議議事録、トランスクリプトを接続することで、未達の「定性的要因」を対話形式で即座に抽出します。
    ...
    """)
    if st.button("次期予測・要因分析ダッシュボードへ進む ➔", type="primary"):
        st.success("（※デモ用：本来はここで要因分析・予測シミュレーションの別ダッシュボードへ遷移します）")

st.markdown("<br>", unsafe_allow_html=True)
# =========================================================================

# 画面を 左側(ダッシュボード) と 右側(AIチャット) に分割
...
with col_dash:
    ...
    with tab3:
        ...
        st.caption("対象スコープ: [Region: EMEA] × [Business: Hardware] の赤字リカバリー")

        base_plan = 25.0  # Hardwareの必達EBITDA目標
        current_act = 5.0 # ゼネコン待ち等による待機費高騰で大幅未達 (Plan 25 -> Act 5)
...
with col_chat:
    ...
    # デモ用の即時分析アクションボタン
    if st.button("💡 デモ: EMEA Hardwareの要因分析を実行", use_container_width=True):
        st.session_state.messages_strategy.append({"role": "user", "content": "EMEAのHardware事業において利益が大幅未達となっている要因を分析して。"})
    ...
    if prompt := st.chat_input("Ask agent..."):
        ...
        with chat_container:
            ...
            with st.chat_message("assistant", avatar="🦅"):
                # デモ用簡易キーワード反応
                if any(word in prompt for word in ["EMEA", "要因", "分析", "なぜ", "未達", "ハード"]):
